import { DiffOp } from "@thenullnode/nulldown/nulledit/types";
import type {
  DropDiffEventMetadata,
  DropDiffNativeOp,
  DropDiffOp,
  JsonValue,
} from "@thenullnode/nulldown/drop/diff";
import { z } from "zod";
...
/** Schema for native diff ranges accepted by the MCP package. */
...
/** Schema for native binary diff operations accepted by the MCP package. */
...
/** Schema for legacy or native diff operations accepted by `diff_apply`. */
...
/** Schema for branch diff event metadata accepted by `diff_apply`. */
