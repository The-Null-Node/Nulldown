...
export class ProviderCrypto {
  ...
  async wrapRawContentKeyForRequester(
    ...
    const requesterPublicKey =
      await this.importRequesterPublicKey(requesterPublicJwk);
...
/** Shared server crypto facade used by route services that need provider keys. */
export const providerCrypto = new ProviderCrypto();
