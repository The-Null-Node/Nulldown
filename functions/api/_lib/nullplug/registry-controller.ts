...
import { timingSafeStringEqual } from "../core/auth/bearer";
...
import type { BlobObject, BlobObjectStore } from "../../../../src/server/ports";

/** Portable storage, authentication, and policy environment for one registry request. */
export interface NullplugRegistryControllerEnv extends AccountAuthEnv {
...
const readRegistryRecord = async (
  object: Pick<BlobObject, "json"> | null,
...
const handleGet = async (
  env: NullplugRegistryControllerEnv,
  request: Request,
): Promise<Response> => {
  ...
  try {
    ...
    const records = await Promise.all(
      listed.objects.map((entry) =>
        env.R2_BUCKET.get(entry.key).then(readRegistryRecord),
      ),
    );
    const items = records
...
/** Handles one request against the portable Nullplug registry HTTP contract. */
export const handleNullplugRegistryRequest = async (context: {
  request: Request;
  env: NullplugRegistryControllerEnv;
}): Promise<Response> => {
