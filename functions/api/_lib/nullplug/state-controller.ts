  ...
  try {
    ...
    const committed = await withBranchMutationLock(
      ...
      async (lock) => {
        ...
        const requestedFact: NullplugUiStateFact = {
          ...parsed,
          source: {
            ...parsed.source,
        ...
        const existingFact = await readNullplugUiStateFact(
          serviceEnv.R2_BUCKET,
          requestedFact,
        );
        ...
        if (existingFact) {
          const normalizedRetry: NullplugUiStateFact = {
            ...requestedFact,
          ...
          if (
            ...
            return {
              error: jsonErrorResponse(
                ...
                "The state identity was already used for different data.",
              ),
            } as const;
          }
          await lock.beginCommit();
          const { key, fact } = await putNullplugUiStateFact(
            serviceEnv.R2_BUCKET,
            existingFact,
            serviceEnv.DB,
          );
        ...
        const candidateFact: NullplugUiStateFact = {
          ...requestedFact,
          createdAt: Date.now(),
          source: {
            ...requestedFact.source,
        ...
        await lock.beginCommit();
        const { key, written, fact } = await putNullplugUiStateFact(
          serviceEnv.R2_BUCKET,
          candidateFact,
          serviceEnv.DB,
        );
    ...
    if (error instanceof BranchMutationLockError) {
      ...
      return jsonErrorResponse(
        ...
        error.outcome === "not_committed"
          ...
          : "Mutation outcome could not be confirmed. Retry the exact same state change.",
      );
    }
