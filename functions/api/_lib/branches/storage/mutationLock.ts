...
): string | undefined => object?.etag ?? object?.httpEtag;
...
const lockBody = (token: string): string =>
...
const recoverOwnedLease = async (
  ...
  try {
    ...
    if (current.payload?.token !== lock.token || !current.etag) {
      return "not_held";
    }
...
const acquireBranchMutationLock = async (
  ...
  for (let attempt = 0; attempt < BRANCH_LOCK_MAX_ATTEMPTS; attempt += 1) {
    ...
    const isStale =
      !existing.payload ||
      Date.now() - existing.payload.createdAt > BRANCH_LOCK_STALE_MS;
...
const releaseBranchMutationLock = async (
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      // Re-read below to reconcile a delete whose response was lost.
    }
    let current: ReadBranchLock;
    try {
      current = await readBranchLock(bucket, lock.key);
    } catch {
      return "unknown";
    }
    if (!current.payload && !current.etag) return "released";
    if (current.payload?.token !== lock.token) return "not_held";
  }
  return "unknown";
};
...
export const withBranchMutationLock = async <T>(
  ...
  const scheduleRenewal = (): void => {
    renewalTimer = setTimeout(() => {
      activeRenewal = (async () => {
        await renewBranchMutationLock(bucket, lock);
      })().finally(() => {
