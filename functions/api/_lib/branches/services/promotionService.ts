...
export const promoteBranchSnapshot = async (
  ...
  try {
    ...
    const result = await withBranchMutationLock(
      ...
      async (lock) => {
        ...
        const receipt: BranchPromotionReceipt = {
          ...
          ...receiptIdentity,
          expectedSnapshotId: promotion.expectedSnapshotId,
          promotedDropId: await reserveRemoteJsonDropId(env.R2_BUCKET, env.DB),
          promotedPayload,
          status: "pending",
          createdAt: Date.now(),
        };
        if (!(await createBranchPromotionReceipt(env.R2_BUCKET, receipt))) {
    ...
    if (error instanceof BranchMutationLockError) {
      return jsonErrorResponse(
        error.outcome === "not_committed" ? 409 : 503,
        error.code,
