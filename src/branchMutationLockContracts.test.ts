...
describe("branch mutation lock", () => {
  ...
  it("does not mask a completed commit when lock ownership is lost during cleanup", async () => {
    const store = createFilesystemBlobStore({ rootDir });
    const key = createBranchLockKey("root-1", "branch-1");
    const warning = jest.spyOn(console, "warn").mockImplementation(() => undefined);
  ...
  it("does not mask a completed commit when ownership cannot be read during cleanup", async () => {
    const delegate = createFilesystemBlobStore({ rootDir });
    let getCount = 0;
    const store: VoidBlobStore = {
      get: async (key) => {
