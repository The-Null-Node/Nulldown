...
export interface CreateRemoteNullplugRuntimeOptions {
  ...
  providerId?: string;
  /** Nulldown API base URL. Empty uses the current origin. */
  baseUrl?: string;
...
export const createRemoteNullplugRuntime = (
  ...
  const loadRegistry = async (): Promise<
    Map<string, RemoteNullplugManifest>
  > => {
...
let defaultRemoteRuntime: NullplugRuntime | null = null;

/** Returns the shared same-origin remote nullplug runtime for browser render surfaces. */
export const getDefaultRemoteNullplugRuntime = (): NullplugRuntime => {
