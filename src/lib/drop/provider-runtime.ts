...
export const compareDropRuntimeVersions = (
  ...
  if (local && remote) {
    if (compareOptionalHash("contentHash", local, remote)) {
      ...
      const snapshotStatus = compareNumbers(
        local.snapshotId,
        remote.snapshotId,
      );
...
export const versionRefFromBranchContent = async (
  ...
  ...(source.metadata !== undefined
    ? {
        metadataHash: await hashMarkdownSource(
          serializeCanonicalJson(source.metadata),
        ),
      }
    : {}),
  ...(source.runtimeHeapHash
    ? { runtimeHeapHash: source.runtimeHeapHash }
    : {}),
});
