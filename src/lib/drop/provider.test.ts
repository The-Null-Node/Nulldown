...
import type { DropCrypto } from "../crypto/browser-drop-crypto";
...
const createCryptoPort = (
  ...
  seal: jest.fn<DropCrypto["seal"]>().mockResolvedValue(envelope),
  open: jest.fn<DropCrypto["open"]>().mockResolvedValue(payload),
});

describe("drop provider ports", () => {
  beforeEach(() => {
    ensureWindowWithIndexedDb();
    (globalThis as { fetch?: typeof fetch }).fetch = jest.fn<typeof fetch>();
  });

  it("creates and reads local drops with createLocalDropProviderPort", async () => {
    ...
    const provider = createLocalDropProviderPort({
      crypto: cryptoPort,
    });
  ...
  it("creates and reads remote drops with createRemoteDropProviderPort", async () => {
    const payload: DropPayload = {
    ...
    const envelope = createEnvelope();
    const cryptoPort = createCryptoPort(payload, envelope);
    const provider = createRemoteDropProviderPort({
      crypto: cryptoPort,
    });
  ...
  it("resolves graph lineage and uses cache on subsequent reads", async () => {
    ...
    const registry = createDropProviderPortRegistry({
  ...
  it("selects provider by drop id with createDropProviderPortRegistry", () => {
    const registry = createDropProviderPortRegistry({
      crypto: createCryptoPort({ content: "unused" }),
    });
