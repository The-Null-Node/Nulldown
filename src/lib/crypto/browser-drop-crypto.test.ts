...
const { BrowserDropCrypto, createBrowserDropCrypto } = await import(
  "./browser-drop-crypto"
);
...
describe("browser void crypto", () => {
  ...
  it("encodes canonical draft packs as V1 wire data before sealing", async () => {
    ...
    await new BrowserDropCrypto({ vault: createVaultMock() as any }).seal({
      content: "Hello from test",
      draftPack,
    });
  ...
  it.each([
    [{ Authorization: "Bearer account-token" }, true],
    [{}, false],
  ])(
    "uses existing account auth headers for escrow unlock (credential: %s)",
    async (authHeaders, hasCredential) => {
      const subtle = installMockCrypto();
      subtle.verify.mockResolvedValue(true);
      subtle.decrypt
        .mockRejectedValueOnce(new Error("local vault mismatch"))
        .mockResolvedValueOnce(Uint8Array.from([1, 2, 3, 4]).buffer)
        .mockResolvedValueOnce(new TextEncoder().encode("opened content").buffer);
      subtle.importKey.mockResolvedValue({} as CryptoKey);
      getAccountAuthHeaders.mockResolvedValue(authHeaders);
      const fetchMock = jest.fn().mockResolvedValue(
        new Response(JSON.stringify({ wrappedKey: "AQIDBA==" }), {
          status: 200,
      ...
      const payload = await new BrowserDropCrypto({
        vault: createVaultMock() as any,
      }).open(createEscrowEnvelope(), { dropId: "PrivateRoot01" });

      expect(payload.content).toBe("opened content");
      expect(getAccountAuthHeaders).toHaveBeenCalledTimes(1);
      ...
      expect((init.headers as Record<string, string>).Authorization).toBe(
        hasCredential ? "Bearer account-token" : undefined,
      );
    },
  );
