...
/** Portable HTTP fetch operation used by file-backed CLI credentials. */
export type CliCredentialFetch = (
...
export const readCliCredential = async (
  ...
  try {
    ...
    return credential
      ? {
          ...credential,
          baseUrl: normalizeCliCredentialBaseUrl(credential.baseUrl),
        }
      : null;
  } catch {
    return null;
...
export const mergeCliCredentialAuthoring = (
  ...
  if (
    ...
    throw new Error(
      "CLI credential refresh changed the delegated authoring identity.",
    );
  }
    ...
    refreshPromise = (async () => {
      ...
      try {
        const response = await fetchImpl(
          `${canonicalBaseUrl}/api/auth/cli/refresh`,
          {
            method: "POST",
        ...
        const body = decodeCliCredentialBundle(
        ...
        if (
          ...
          error.message === "Nulldown credential persistence failed."
        ) {
          throw error;
        }
        throw new Error("Nulldown credential refresh failed.");
