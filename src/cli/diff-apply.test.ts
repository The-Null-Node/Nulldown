...
describe("CLI diff apply", () => {
  it("accepts the server-selected branch receipt when --branch is omitted", async () => {
    ...
    const fetchImpl = jest.fn<CliFetch>(async (input, init) => {
      ...
      expect(url).toBe("http://example.test/api/diff/root-1");
      expect(init?.method).toBe("POST");
      const eventId = JSON.parse(String(init?.body)).events[0].eventId;
      return Response.json({
        accepted: 1, deduplicated: 0, branchId: "clone_anonymous",
        snapshotId: 1, totalStored: 1,
        acknowledgements: [{ eventId, seq: 0, snapshotId: 1, status: "accepted" }],
      });
    });
    const result = await runCli([
      "diff", "apply", "root-1", "--event-id=retry-default-branch",
      "--created-at=1725000000000", "--insert=0:hello",
      "--base=http://example.test", "--json",
    ], { fetch: fetchImpl, stdout: (text) => stdout.push(text), stderr: (text) => stderr.push(text) });
    expect(stderr).toEqual([]);
    expect(result).toEqual({ exitCode: 0 });
    expect(fetchImpl).toHaveBeenCalledTimes(2);
    expect(JSON.parse(stdout[0]!)).toMatchObject({ accepted: 1, branchId: "clone_anonymous" });
  });
