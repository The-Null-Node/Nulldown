...
const requireDiffAppendReceipt = (
  ...
  if (request.branchId && value.branchId !== request.branchId) {
    throw new Error(
      "Diff response did not acknowledge the requested branch. Retry the exact same envelope.",
    );
  }

  for (const eventId of new Set(
    request.envelope.events.map((event) => event.eventId),
  )) {
    if (
      value.acknowledgements.filter(
        (acknowledgement) => acknowledgement.eventId === eventId,
      ).length !== 1
    ) {
      throw new Error(
        "Diff response did not acknowledge every submitted event. Retry the exact same envelope.",
      );
    }
  }
  return value;
