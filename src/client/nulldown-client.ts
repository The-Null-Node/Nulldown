...
export interface NulldownBranchQueryRequest {
  /** Approximate response token budget forwarded to compact projections. */
  maxTokens?: number;
  /** Whether compact projections should use preview text. */
  preview?: boolean;
  /** Root drop id. */
  rootId: string;
  /** Branch id. */
...
/** Nullplug UI state facts accepted by the backend Nullplug routes. */
export type NulldownNullplugStateFact =
  NullplugUiStatePatchFact | NullplugUiStateSnapshot;
...
const decodeDropReadBody = (body: unknown): unknown => {
  ...
  if (looksLikeSealedEnvelope(body)) {
    throw new NulldownClientError(
      "Drop response contains an invalid sealed envelope.",
    );
  }
...
export const createNulldownClientConfig = (
  ...
  token: Object.hasOwn(options, "token")
    ? options.token
    : (process.env.ND_TOKEN ?? null),
...
export class NulldownClient {
  ...
  async request<T = unknown>(
    ...
    const explicitAuthorization = new Headers(options.headers).has(
      "Authorization",
    );
  ...
  async readStrategy(
    ...
    const root =
      branchId === undefined ? await this.getDrop(request.id) : undefined;
    if (
      root &&
    ...
    if (branchId !== undefined) {
      ...
      if (
        ...
        (root === undefined &&
          ...
          !(
            isShortDropId(request.id) &&
            toShortDropId(data.rootDropId) === request.id
          )) ||
  ...
  /** Resolves a Nullplug invocation through the backend runtime. */
